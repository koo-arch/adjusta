...
Google Calendarを見ながら複数の候補日程を管理し、日程の確定までを一つの流れで進められるWebアプリケーションです。
...
Google Calendarの予定を確認しながら複数の候補を作成し、相手から回答を受けた後は、その中から日時を確定できます。必要に応じて候補予定を専用カレンダーへ表示し、確定した予定はメインカレンダーへ反映します。
...
5. 確定した予定をGoogle Calendarへ登録・更新する
...
| カレンダー表示 | Google Calendarの予定とAdjustaのイベントを確認する |
| イベント管理 | 下書きの作成、一覧・詳細表示、編集、削除を行う |
| 候補日程管理 | 複数候補の追加、日時変更、並び替え、削除を行う |
| 日程確定 | 候補の中から日時を確定し、Google Calendarへ反映する |
...
実際の画面を掲載できるよう、次の画面キャプチャを追加する予定です。
...
現時点では[本番環境](https://adjusta.vercel.app)から公開画面を確認できます。
...
### Google Calendarへの重複登録を防ぐ同期処理

候補予定ごとに同期状況、Google Calendar上の予定ID、最終同期日時、エラー内容を記録しています。一度同期した内容は、イベントが編集されるまで再登録しないことで、同じ予定が重複して作成されることを防ぎます。
...
Playwrightで、公開画面、認証、イベントの作成・編集・詳細・確定・削除、カレンダー設定などの主要な操作を確認しています。自動テストでは実際のGoogleアカウントやGoogle Calendarを変更せず、テスト用Backendを使用します。
...
HTTPハンドラー、ミドルウェア、業務ルール、処理フロー、Google Calendarとの通信を対象にテストしています。認証・権限、入力検証、同期失敗、再試行、重複登録の防止などを確認します。
