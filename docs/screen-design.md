...
実装メモ(2026-07-12): **カレンダー主役+右パネル**のレイアウトで実装(作成フォームと同じ構図。コンテナはヘッダーと同じ `max-w-screen-2xl`、2 カラムは lg から。DESIGN.md 5. Container 参照)。右パネルは**タブ(対応が必要 / 直近の予定)**で、直近に項目があるときはタブに目印ドットを表示。各タブの中身は行カードの縦リスト(先頭 5 件+「すべて見る」→ ステータス絞り込み済み /events)で、loading(スケルトン)/ error(再試行)/ empty を描き分け。カレンダーは**閲覧専用**(選択・ドラッグ・リサイズ無効。ui-review P1 #3)。**イベントクリックの詳細は、lg 以上はクリックした要素に anchor した popover、それ未満は dialog** で表示する(クリック位置の近くに反応を出す。2026-07-12 に詳細タブ方式から変更)。スライダー(Splide)は廃止。
