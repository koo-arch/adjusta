...
実装メモ(2026-07-12): **カレンダー主役+右パネル**のレイアウトで実装(作成フォームと同じ構図。コンテナはヘッダーと同じ `max-w-screen-2xl`、2 カラムは lg から。DESIGN.md 5. Container 参照)。右パネルに「対応が必要なイベント」「直近の予定」を行カードの縦リスト(先頭 5 件+「すべて見る」→ /events)で表示し、各セクションは loading(スケルトン)/ error(再試行)/ empty を描き分け。カレンダーは**閲覧専用**(選択・ドラッグ・リサイズ無効。ui-review P1 #3)で、イベントクリックの詳細表示は shadcn Dialog。スライダー(Splide)は廃止。
