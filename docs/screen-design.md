...
実装メモ(2026-07-12): **カレンダー主役+右パネル**のレイアウトで実装(作成フォームと同じ構図。コンテナはヘッダーと同じ `max-w-screen-2xl`、2 カラムは lg から。DESIGN.md 5. Container 参照)。右パネルは**タブ(対応が必要 / 直近の予定)**で、直近に項目があるときはタブに目印ドットを表示。各タブの中身は行カードの縦リスト(先頭 5 件+「すべて見る」→ ステータス絞り込み済み /events)で、loading(スケルトン)/ error(再試行)/ empty を描き分け。カレンダーは**閲覧専用**(選択・ドラッグ・リサイズ無効。ui-review P1 #3)で、**イベントをクリックするとモーダルではなく右パネルが詳細表示に切り替わる**(× でタブに復帰。縦積み時はパネルへスムーズスクロール)。スライダー(Splide)は廃止。
