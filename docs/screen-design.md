...
実装メモ(2026-07-10): 編集画面(5.7)と同じフォーム改修を適用(直接入力の併設・週/月ビュー切替・shadcn 入力・CSS 分岐・上下ボタン付き並び替え)。登録成功で詳細へ遷移(従来どおり)。カレンダー選択(登録先カレンダー)は引き続き未実装(移行課題 9.6)。

実装メモ(2026-07-11): **ステップ型レイアウト**に変更。カレンダーを常設(md 以上で左 6)し、隣のパネル(右 4)がステップで「1 基本情報 ⇄ 2 候補日程」に切り替わる(ステッパーはクリックで行き来可、ステップ状態は Jotai)。候補日程パネル上部には**基本情報のサマリー**(タイトル・場所+編集リンク)を置き、確認ステップの代替とする(独立した確認ステップは設けない=ユーザー判断)。送信バーは下部 sticky で、ステップ1=「次へ」、ステップ2=「戻る+送信」。送信時に基本情報側のエラーが出た場合はステッパーを赤くし、バーの警告クリックでステップ1へ戻れる。FullCalendar は DESIGN.md の `--fc-*` トークンを適用。入力欄に placeholder 例、モバイルはパネル→カレンダーの縦積みでカレンダーは日ビュー開始。
