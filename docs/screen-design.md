...
| データ | エンティティ | API |
| --- | --- | --- |
| イベント一覧 | events(+ proposed_dates の要約) | `GET /api/calendar/event/draft/list` |
...
検索・絞り込み(要件 8.4 に記載)は Could のため初期対象外とする(8章・移行課題 9.4 参照)。
...
- **Phase 1(表示系)**: プロフィール / Google 連携 / ログアウト(既存 API のみで実装可。`ui-review.md` バックログ #15 に対応)
...
| 検索・絞り込み(要件 8.4 の一部、要件 9.3) | `/events` 内 | ステータス・キーワードによる絞り込み。バックエンドには `GET /api/event/draft/search` が既に存在する |
| 一覧のページネーション(要件 9.3) | `/events` 内 | 件数増加時に一覧系 API・UI をページネーション対応にする(方式は設計書で定義) |
