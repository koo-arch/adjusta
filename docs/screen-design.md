...
実装メモ(2026-07-11): レイアウトを「基本情報 → 候補日程」の縦ステップ型に変更。カレンダー(操作)と選択済みリスト(結果)は**「候補日程」1 セクションに統合**(見出し直下に説明+「日時を追加」、内側でカレンダー 6 / リスト 4 の 2 カラム)。送信ボタンとフォームエラーは**下部 sticky バー**に配置。FullCalendar は DESIGN.md の `--fc-*` トークンを適用。入力欄に placeholder 例、モバイルはカレンダーを日ビューで開始。
