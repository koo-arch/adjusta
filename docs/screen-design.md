...
実装メモ(2026-07-10): ヘッダー(タイトル+バッジ+編集/削除)/ 候補日程(コピー+確定 CTA+優先順リスト、確定済みは確定日時を強調し他候補は畳む)/ 基本情報(場所・説明・同期状態)の 3 ブロック構成で実装(`ui-review.md` 3.1)。設計との差分 2 点: ①**登録先カレンダー名は未表示**(詳細 API のレスポンスにカレンダー名がなく、9.6 のカレンダー選択 API 整備と併せて対応)。②**同期失敗時の再試行導線は未実装**(再同期 API が未整備のため last_sync_error の表示まで。導線は API 整備後に追加)。
