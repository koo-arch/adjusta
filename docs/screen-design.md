...
- API が `code = google_reauthorization_required`を返した場合(HTTP 409): ログイン失効とは区別し、再認可導線のモーダル(`AuthErrorModal`)を表示する。Adjusta セッションは維持したまま Google OAuth の再認可へ進み、完了後は元の画面へ戻す
...
- **再認可ボタン**: `code = google_reauthorization_required`時の `AuthErrorModal` と同一の再認可フローに接続する(導線を一元化し、この画面独自のフローを作らない)。再認可では Adjusta からログアウトせず、現在連携中の Google アカウントのトークンのみ更新する
