...
- backend のエラーボディは `{"error": "<message>"}` のみで機械可読コードを持たないため、frontend は暫定的に HTTP 409 で `google_reauthorization_required` を判定している。エラーボディへの `code` フィールド追加を後続課題とする
...
3. backend のエラーボディに機械可読な `code` フィールドを追加し、frontend の `google_reauthorization_required` 判定を HTTP 409 依存から厳密化する
...
7. `cmd/migrate` は当面 ent auto migration の明示実行 CLI とし、破壊的変更が必要になった段階で versioned migration への差し替えを検討する
