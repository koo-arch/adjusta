...
カレンダー用途は role によって表現し、候補予定の Google Calendar 同期有無は sync_proposed_dates によって判断する。

### 2.6 明示的な中間テーブルの採用

user_calendars は、単なる中間テーブルではなく、role、is_visible、sync_proposed_dates などの属性を持つ。
...
| sync_proposed_dates | boolean | NO |  | 候補予定を Google Calendar に同期するか |
| created_at | timestamp | NO |  | 作成日時 |
| updated_at | timestamp | NO |  | 更新日時 |
| deleted_at | timestamp | YES |  | 論理削除日時 |
...
* priority は内部用の順序キーとして扱い、数値が大きいほど高優先とする。
* priority は event ごとに疎な値で保持し、並び替え時は前後の中間値を使って更新する。
* priority の gap が不足した場合のみ、その event 内で priority を再採番する。
* ユーザーへの表示上の優先順位は、priority の降順結果に対して 1, 2, 3... の順位として表現する。
* user_calendars.sync_proposed_dates = false の場合、google_event_id は NULL になり得る。
...
| proposed_dates(event_id, priority) WHERE deleted_at IS NULL | Partial UNIQUE | 同一イベント内で未削除の候補日程 priority 重複を防止する |
...
ON user_calendars (user_id)
WHERE role = 'primary';
CREATE UNIQUE INDEX uq_proposed_date_priority_per_event
ON proposed_dates (event_id, priority)
...
user_calendars.sync_proposed_dates = false の場合、以後は Google Calendar に候補予定を新規作成・更新・再作成せず、Adjusta 側の DB のみで管理する。
