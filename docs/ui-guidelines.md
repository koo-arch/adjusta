...
- **ダークモード**: **ライト固定に決定(2026-07-09)**。`dark:` バリアントを新規に書かない。ThemeProvider / ThemeButton は撤去済み。削除予定の自作プリミティブに残る `dark:` は新たに修正せず、置換時にファイルごと削除する
...
- **認証エラー**: 401 はフルリロード、`code = google_reauthorization_required`は `AuthErrorModal`(`screen-design.md` 4.1 参照)。個別画面で独自ハンドリングしない
...
2. ~~**テーマ有効化経路の整理**~~ **解消済み(2026-07-13)**: ThemeButton / ThemeProvider、`next-themes` 依存を撤去。自作プリミティブの `dark:` はレガシーコンポーネント一掃時に削除
