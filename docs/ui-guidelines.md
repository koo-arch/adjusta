...
| アイコン | @heroicons/react(`/20/solid` を第一候補とする) |
...
| 日時選択 | `DateTimePicker`(react-datepicker) | 存置(テーマは DESIGN.md「Third-Party Components」に合わせる) |
| モーダル | `Modal` | `dialog` |
...
- アイコンは `@heroicons/react/20/solid` を第一候補とし、無い場合のみ react-icons を使う(shadcn 標準の lucide-react を採用する場合は本行を更新すること)
...
5. **存置サードパーティのテーマ統一**: FullCalendar / react-datepicker / Splide は `frontend/DESIGN.md`「4. Third-Party Components」に従いテーマ上書きでトークンに統一する。上書き CSS は `globals.css` のサードパーティ節に集約し、コンポーネント内に散らさない
...
3. **テーマ系統の二重化**: `globals.css` の CSS 変数(`--foreground-rgb` 等)は `prefers-color-scheme` ベースで実質未使用。ライト固定決定に伴い削除候補
