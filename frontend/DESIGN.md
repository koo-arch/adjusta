...
1. **和文フォント未導入**: 現状は Inter のみ。Noto Sans JP を next/font で追加し、Tailwind の `fontFamily` に登録する
...
4. **ページ背景**: 現実装は白ベース。本書は Background `#f9fafb` + Surface `#ffffff` の面分離を規定
...
7. **行間・字間**: 現実装は Tailwind デフォルト行間のまま。本書の 1.7 / 0.02em を base スタイルとして導入する
8. **状態設計・a11y**: loading / error / empty の3状態、label 関連付け、タッチターゲット 44px は `../docs/ui-review.md` のバックログとして実装する
9. **shadcn/ui への段階的移行**(2026-07-09 決定): コンポーネント実装は `src/components/ui/` の shadcn/ui へ順次置換する。本書のトークンは CSS 変数(例: `--primary: #6366f1`)として `globals.css` に定義し、shadcn のテーマ変数に接続する。移行規定は `../docs/ui-guidelines.md` 3.5 を参照
