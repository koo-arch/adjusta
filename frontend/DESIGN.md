...
- **セカンダリ操作(編集・削除・コピー・並べ替え等)は背景色のない ghost アイコンボタン**にする(`aria-label` + `title` 必須)。ラベル付き outline ボタンを並べる配置は「管理画面的で無機質」になるため避ける(2026-07-10 決定)
- **フラット画面(6. サーフェス戦略で囲いを使わない画面)では、主 CTA も塗りつぶしにせず primary 色のテキストボタン**(ghost + text-primary)にする — 塗りつぶしはフラットな画面から浮く(2026-07-10 決定)。フォーム送信のような従来型フォームの submit は塗りつぶしのままでよい
  - **例外(2026-07-15 決定)**: 操作系が1行に集約されたツールバー(イベント一覧のフィルタ+検索+作成)では、主 CTA を primary 塗りにしてよい。隣接要素が互いの錨になるため浮かない。単独配置の CTA(ページヘッダー右端など)は引き続き塗りにしない
- **primary(塗り)は1ビューに1個まで**。イベント一覧では「新規作成」のみ(2026-07-15 決定)
- 破壊的操作(削除)は Danger 色 + 確認モーダル必須
- disabled 時は opacity 50% + `cursor-not-allowed`

### FAB(モバイル作成導線。2026-07-15 決定)

- イベント一覧の作成アクションは md 未満で FAB に移す(`md:hidden`)
- 位置: `fixed right-4 z-40`、`bottom-[calc(1rem+env(safe-area-inset-bottom))]`
- 形状: `size-14 rounded-full shadow-lg`、primary 塗り、アイコンのみ(`aria-label="新規作成"` 必須)
- リストコンテナに `pb-24` を確保し、ページネーション・最終行との重なりを防ぐ
- ツールバーのボタンとロジック(遷移先・ラベル)を共有し、見た目だけ分岐する

### Global Navigation(2026-07-15 決定)

- ナビタブは「場所」(ホーム / イベント一覧)のみとし、アクションをタブにしない
- 作成アクションはヘッダー右側(userMenu 隣)の **ghost アイコンボタン**(`＋`、`aria-label` + `title` 必須)として全ページ・全ブレークポイントに置く — どの画面からも作成に到達できることを保証する補助導線
- 一覧ページ内の主導線(ツールバーの primary / FAB)とは役割を分け、ヘッダー側は目立たせない
...
| 768px(`max-w-screen-md`) | リスト中心・読む画面(アカウント、詳細) |
| 1152px(`max-w-6xl`) | **カードグリッドの一覧画面**(イベント一覧)。3 カラムのカードが収まる幅(2026-07-15 決定) |
| 1536px(`max-w-screen-2xl`) | **カレンダー主役のワークスペース画面**(ダッシュボード、作成・編集)。ヘッダーのコンテナ幅と揃える。右パネル(24rem)との 2 カラム化は **lg(1024px)から**(それ未満はカレンダーが潰れるため縦積み) |

- Padding (horizontal): 16px(ワークスペース画面は md 以上 32px = ヘッダーと同じ。イベント一覧は sm 以上 24px = `px-4 sm:px-6`)
- 横スクロールする行要素(フィルタチップ等)は `-mx-4 px-4 sm:-mx-6 sm:px-6 md:mx-0 md:px-0` で画面端までブリードさせ、続きがあることを示す(2026-07-15 決定)
...
2. ~~**テーマ有効化経路の整理**~~ **解消済み(2026-07-14)**: ThemeButton / ThemeProvider / `next-themes` と旧プリミティブの `dark:` を撤去し、ライト固定へ統一した
3. ~~**ボタン radius の変更**~~ **解消済み(2026-07-14)**: shadcn `button` の `rounded-md`(6px)へ統一
4. ~~**ページ背景**~~ **解消済み(2026-07-09)**: body に Background `#f9fafb`(`bg-background`)を適用し、Surface はカード側(`bg-card`)で分離
5. ~~**未定義トークンの解消**~~ **解消済み(2026-07-14)**: 旧 `ToggleSwitch` を削除し shadcn `switch` へ統一
6. ~~**`text-md` の排除**~~ **解消済み(2026-07-14)**: 該当する旧入力プリミティブを削除
...
9. ~~**shadcn/ui への段階的移行**~~ **完了(2026-07-14)**: 旧自作プリミティブを削除し、`ui/` の shadcn プリミティブと `common/` の共通組み立てへ統一。Headless UI・Heroicons・react-toastify の依存も撤去
10. ~~**サードパーティ UI のテーマ未統一**~~ **解消済み(2026-07-14)**: FullCalendar はトークン接続済み、react-datepicker と Splide は置換・廃止、通知は sonner へ統一
