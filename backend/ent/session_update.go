...
func (su *SessionUpdate) check() error {
	if v, ok := su.mutation.SessionToken(); ok {
		if err := session.SessionTokenValidator(v); err != nil {
			return &ValidationError{Name: "session_token", err: fmt.Errorf(`ent: validator failed for field "Session.session_token": %w`, err)}
		}
	}
...
func (suo *SessionUpdateOne) check() error {
	if v, ok := suo.mutation.SessionToken(); ok {
		if err := session.SessionTokenValidator(v); err != nil {
			return &ValidationError{Name: "session_token", err: fmt.Errorf(`ent: validator failed for field "Session.session_token": %w`, err)}
		}
	}
