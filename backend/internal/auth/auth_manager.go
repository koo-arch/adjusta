...
func (am *AuthManager) VerifyOAuthToken(ctx context.Context, userID uuid.UUID) (*oauth2.Token, error) {
	entAccount, err := am.accountRepo.FindByUserID(ctx, nil, userID)
	if err != nil {
		log.Printf("failed to get account by user id: %v", err)
		if ent.IsNotFound(err) {
			return nil, internalErrors.NewAPIError(http.StatusNotFound, "アカウント情報が見つかりませんでした")
	...
	if newToken.AccessToken != token.AccessToken {
		...
		_, err = am.accountRepo.Update(ctx, nil, entAccount.ID, buildAccountTokenRefreshOptions(entAccount.GoogleUserID, newToken, entAccount.Scope))
		if err != nil {
...
func buildAccountTokenRefreshOptions(googleUserID string, oauthToken *oauth2.Token, currentScope *string) repoAccount.AccountMutationOptions {
